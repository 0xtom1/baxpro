-- Update v_asset_summary image_url to use metadata_json image with fallback to bottle_release image_url

-- Drop materialized view first (depends on the view)
DROP MATERIALIZED VIEW IF EXISTS baxus.mv_brands_list;

-- Recreate v_asset_summary with COALESCE for image_url
...
SELECT 
  ...
  COALESCE(a.metadata_json ->> 'image', a.asset_json -> 'bottle_release' ->> 'image_url') AS image_url,
...
-- Create unique index on brand_name (required for CONCURRENTLY refresh)
