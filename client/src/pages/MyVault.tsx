...
type VaultTab = "bottles" | "loans";

export default function MyVault() {
  const { user, loading: authLoading } = useRequireAuth();
  usePageTitle("My Vault");
  const [, setLocation] = useLocation();
  const [activeTab, setActiveTab] = useState<VaultTab>("bottles");
  ...
  const tabs = [
    { id: "bottles" as VaultTab, label: "MY BOTTLES", icon: Package },
    { id: "loans" as VaultTab, label: "MY LOANS", icon: Landmark },
  ];
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="border-b border-border px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
        <div className="flex items-center gap-1">
          ...
          {activeTab === "loans" && user?.phantomWallet && (
            <Button
              size="sm"
              onClick={() => setLocation('/create-loan')}
              data-testid="button-create-loan-from-loans"
            >
              <Landmark className="w-4 h-4 mr-2" />
              Create Loan
            </Button>
